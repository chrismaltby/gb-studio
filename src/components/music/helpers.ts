...
const patternCelltoString = (
  ...
  return `|${
    fields.includes("note") ? renderNote(p.note) : "   "}${
    ...
    .map((r, i) => {
      ...
      if (r[0] === "|") {
        ...
        return channel.map((c, j) => {
          ...
          patternCell.note =        cellString[0] !== "   " ? (note === -1 ? null : note) : -9;
          patternCell.instrument =  cellString[1] !== "  "  ? strToInt(cellString[1], 10, -1) : -9;
          patternCell.effectcode =  cellString[2] !== " "   ? strToInt(cellString[2], 16) : -9;
          patternCell.effectparam = cellString[3] !== "  "  ? strToInt(cellString[3], 16) : -9;
          return patternCell;
        });
      }
