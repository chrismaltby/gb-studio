...
import { getScenes, getScenesLookup, getProcedures, getActorsLookup, getTriggersLookup } from "../reducers/entitiesReducer";
...
export default store => next => action => {
  if (action.type === OPEN_HELP) {
    ...
    const state = store.getState();
    const procedure =
      state.entities.present.entities.procedures[action.procedureId];
    const procedureIndex = getProcedures(state).indexOf(procedure);
    const procedureName = procedure.name || `${l10n("CUSTOM_EVENT")} ${procedureIndex + 1}`
    ...
    const isThisEvent = (event) =>
      event.command === EVENT_CALL_PROCEDURE &&
      event.args.procedure === action.procedureId;
    ...
    const usedTotal = usedScenes.length + usedActors.length + usedTriggers.length;

    if (usedTotal > 0) {
      const sceneNames = uniq([].concat([], usedScenes, usedActors, usedTriggers)
        .map(([sceneId]) => sceneName(sceneId)))
        .sort();
      
      // Display confirmation and stop delete if cancelled
      const cancel = confirmDeleteCustomEvent(procedureName, sceneNames, usedTotal);
      if (cancel) {
        return;
      }
