...
export interface ScriptUsesInput {
  scriptId: string;
...
workerCtx.onmessage = createWorkerRequestHandler<ScriptUsesInput, ScriptUse[]>(
  workerCtx,
  (input) => {
    ...
    Object.values(customEventsLookup).forEach(
      (customEvent, customEventIndex) => {
