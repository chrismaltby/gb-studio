...
void adventure_update() BANKED {
    ...
    while (solid_count) {
        ...
        if ((sol_actor->solid & COLLISION_X)) { // Verify the solid can be collided with on the X axis
            if (bb_intersects_opt(&PLAYER.bounds, &sol_pos, &sol_actor->bounds, &sol_actor->pos)) { // Check if the player's and the solid's bounding boxes overlap
                if ((sol_actor->solid & COLLISION_LEFT) && ((sol_pos.x) < (sol_actor->pos.x))) { // Verify that the solid has left collisions enabled and that the player is to the left of the solid
                    if (abs(PLAYER.pos.x - (sol_actor->pos.x + ((sol_actor->bounds.left - PLAYER.bounds.right) << 4))) <= (abs(pl_vel.x) + abs(sol_actor->vel.x))) { // Prevent too-drastic position correction
                ...
                } else if ((sol_actor->solid & COLLISION_RIGHT) && ((sol_pos.x) > (sol_actor->pos.x))) { // Verify that the solid has right collisions enabled and that the player is to the right of the solid
                    if (abs(PLAYER.pos.x - (sol_actor->pos.x + ((sol_actor->bounds.right - PLAYER.bounds.left + 1) << 4))) <= (abs(pl_vel.x) + abs(sol_actor->vel.x))) { // Prevent too-drastic position correction
        ...
        if ((sol_actor->solid & COLLISION_Y) && sol_actor != hit_actor) { // Verify the solid can be collided with on the Y axis and that we haven't already collided on the X axis
            if (bb_intersects_opt(&PLAYER.bounds, &sol_pos, &sol_actor->bounds, &sol_actor->pos)) { // Check if the player's and the solid's bounding boxes overlap
                if ((sol_actor->solid & COLLISION_TOP) && ((sol_pos.y) < (sol_actor->pos.y))) { // Verify that the solid has top collisions enabled and that the player is above the solid
                    if (abs(PLAYER.pos.y - (sol_actor->pos.y + ((sol_actor->bounds.top - PLAYER.bounds.bottom) << 4))) <= (abs(pl_vel.y) + abs(sol_actor->vel.y))) { // Prevent too-drastic position correction
                        new_pos.y = sol_actor->pos.y + ((sol_actor->bounds.top - PLAYER.bounds.bottom) << 4) - 1; // Position the player above the solid
                        hit_actor = sol_actor; // Log the solid actor we're colliding with
                    }
                } else if ((sol_actor->solid & COLLISION_BOTTOM) && ((sol_pos.y) > (sol_actor->pos.y))) { // Verify that the solid has bottom collisions enabled and that the player is below the solid
                    if (abs(PLAYER.pos.y - (sol_actor->pos.y + ((sol_actor->bounds.bottom - PLAYER.bounds.top + 1) << 4))) <= (abs(pl_vel.y) + abs(sol_actor->vel.y))) { // Prevent too-drastic position correction
    ...
    // Apply the new, post-collision calculated position to the player
