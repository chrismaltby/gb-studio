...
UINT8 FadeInOp(UINT16 c, UINT16 i) { return U_LESS_THAN(c, i) ? 0 : (c - i); }
...
void FadeDMG(UINT8 fadeout) {
  ...
  for (i = 0; i != 4; ++i) {
    ...
    for (j = 0; j != 3; ++j) {
      c = &colors[j << 2];
      *pals[j] =
          PAL_DEF(FadeInOp(c[0], p), FadeInOp(c[1], p), FadeInOp(c[2], p), FadeInOp(c[3], p));
    }
    delay(50);
  }
}

void FadeInDMG() { FadeDMG(0); }

UWORD UpdateColor(UINT8 i, UWORD col) {
  // return RGB2(DespRight(PAL_RED(col), i), DespRight(PAL_GREEN(col), i), DespRight(PAL_BLUE(col),
  // i));
  return RGB2(PAL_RED(col) | DespRight(0x1F, 5 - i), PAL_GREEN(col) | DespRight(0x1F, 5 - i),
              PAL_BLUE(col) | DespRight(0x1F, 5 - i));
}
