...
import { autoFlipTileData, autoFlipTiles } from "shared/lib/tiles/autoFlip";
...
const buildTileData = async (
  ...
  const sourceTileData = await readFileToTilesDataArray(filename);
  const { tileData, tileAttrs, tilesetData } = autoFlipTileData({
...
test("Should auto flip raw tile data to create optimised tileset", async () => {
  const filename = `${__dirname}/_files/test/autoflip1.png`;
  const { tileData, tileAttrs, uniqueTiles } = await buildTileData(
    filename,
    [],
    [],
  );
...
test("Should keep tile palette data when flipping raw tile data", async () => {
  const filename = `${__dirname}/_files/test/autoflip1.png`;
  const { tileData, tileAttrs, uniqueTiles } = await buildTileData(
    filename,
    [0, 1, 2, 3, 4, 5, 6, 7],
...
test("Should keep tile priority data when flipping raw tile data", async () => {
  const filename = `${__dirname}/_files/test/autoflip1.png`;
  const { tileData, tileAttrs, uniqueTiles } = await buildTileData(
    filename,
    [PRIORITY, 0, PRIORITY, 0, PRIORITY, 0, PRIORITY, 0],
...
test("Should prioritise common tiles when flipping raw tile data", async () => {
  const filename = `${__dirname}/_files/test/autoflip1.png`;
  const commonFilename = `${__dirname}/_files/test/autoflip1_common.png`;
  const commonTileData = await readFileToTilesDataArray(commonFilename);
  const { tileData, tileAttrs, uniqueTiles } = await buildTileData(
