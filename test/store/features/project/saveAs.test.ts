import reducer, { initialState } from "../../../../src/store/features/document/documentState";
...
test("Should cause project to exit saving state without changing anything", () => {
  ...
  const action = actions.saveProject.rejected(new Error("Failed"), "randomid", "randomid");
  const newState = reducer(state, action);
...
test("Should change the path and root to new path and root and update modified state", () => {
  ...
  const action = actions.saveProject.fulfilled(undefined, "randomid", "new_test_root/project_copy.gbsproj");
  const newState = reducer(state, action);
  expect(newState.saving).toBe(false);
